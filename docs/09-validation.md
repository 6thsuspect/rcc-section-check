...
- V3 is computed by `auditCovers` in `src/engine/cover.ts`: every bar is classified against
  the nearest face of the actual polygon (outer or void), then compared with that face's
  nominal cover. The result drives the compliance row "Nominal cover achieved — per face",
  the red rings on short bars in the preview, and the per-bar cover readout on hover. A
  1 mm allowance is added on curved (polygonised) faces, scaling with the segment sagitta,
  so a circular layout is not flagged by the polygon approximation.
